...
def extract_with_openai(settings: IRPipelineSettings, document: Document, candidates: list[ExtractedSignal]) -> list[ExtractedSignal]:
    ...
    prompt = {
        ...
        "instruction": "公開IR資料の候補本文からCRE営業仮説シグナルをJSON配列で抽出。断定表現を避ける。各要素はsignal_type, summary, evidence_text, source_document, source_url, confidence, recommended_sales_actionを含める。evidence_text必須。",

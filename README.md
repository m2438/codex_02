...
- 有価証券報告書: EDINET APIの「書類一覧API」で対象書類を検索し、「書類取得API」で取得する設計です。
- 中期経営計画書、統合報告書、決算説明資料、サステナビリティレポート: `documents.source_url` に手動登録されたURLから取得します。
...
- `POST /api/companies/{company_id}/analyze` — 保存済み/登録済みテキストからCRE関連箇所抽出・シグナル生成

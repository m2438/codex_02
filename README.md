...
> 現在の `main` は、営業デモ版として扱える状態を想定しています。日本国内の実在上場企業10社の公開IR情報を対象に、資料取得、本文抽出、CREシグナル分析、営業仮説レポート確認までを一連の流れで説明できます。
...
営業デモでは、画面上の章番号に沿って以下の順番で説明します。

1. 企業ランキングで企業を選択します。営業優先度スコア順に並ぶため、上位企業から説明すると比較しやすくなります。
2. 企業詳細で、本社、従業員数、売上高、EDINETコードを確認します。
3. 「財務関連指標」で、売上成長率、営業利益率、設備投資額、現預金等を確認し、投資余力と事業変化を説明します。
4. 「営業優先度スコア」で、総合点とコンポーネント別スコアを確認します。
5. 「資料取得を実行」を押し、登録済みIR URLまたはEDINETから取得対象を確認します。
6. 「分析実行」を押し、取得済みPDF本文または保存済みIR本文からCREシグナルを更新します。
7. 「分析レポート」で、エグゼクティブサマリー、スコア内訳、CRE需要兆候、財務所見、提案テーマ、追加ヒアリング事項を確認します。
8. 「根拠資料」で、参照IR資料、出典URL、EDINET docID、取得・本文抽出状況を確認します。

### Windows コマンドプロンプトでの起動例

```cmd
copy .env.example .env
set EDINET_API_KEY=your_edinet_api_key_here
set OPENAI_API_KEY=your_openai_api_key_here
set IR_FETCH_MAX_FILE_MB=30
docker compose up --build
```

`OPENAI_API_KEY` は任意です。未設定の場合も、保存済みデータとルールベース分析で画面確認できます。実キーは `.env`、README、ソースコード、スクリーンショット、PR本文に記載しないでください。

### 資料取得・分析の確認ポイント

- PDFサイズ超過の場合: `IR_FETCH_MAX_FILE_MB` を引き上げるか、対象資料URLが適切か確認してください。
- PDFリンク未発見の場合: 登録URLがPDF直リンクではない可能性があります。IR資料ページ内のPDFリンクを確認してください。
- EDINET docID未発見の場合: EDINETコード、書類種別、検索期間、`EDINET_API_KEY` の設定を確認してください。
- AI分析に失敗する場合: `OPENAI_API_KEY` の設定、ネットワーク接続、バックエンドログを確認してください。キー本体は画面に表示されません。
- 画面が更新されない場合: 資料取得または分析実行後、企業詳細のステータスと分析レポート欄を確認し、必要に応じてブラウザを再読み込みしてください。
...
| `OPENAI_API_KEY` | OpenAI API キー。未設定時はルールベース分析または保存済みデータで動作します。フロントエンドには露出しません。 | 空 |
| `EDINET_API_KEY` | EDINET API キー。EDINET 書類一覧・書類取得を使う場合に設定します。 | 空 |
| `IR_FETCH_MAX_FILE_MB` | 取得PDFの最大サイズ（MB）。大きいIR資料を扱う場合は調整します。 | `20` |
...
現在の `main` を営業デモ版として固定する場合は、ローカルで最終確認後に以下のようなタグを作成してください。Codex が自動でタグを作成する必要はありません。
...
| `IR_FETCH_MAX_FILE_MB` | PDF取得時のファイルサイズ上限（MB）です。未設定時は100MBです。 |
...
PDFサイズ上限は通常 `IR_FETCH_MAX_FILE_MB=100`（100MB）を推奨します。統合報告書やサステナビリティレポートなどで必要な場合のみ150MB程度までを目安に引き上げてください。上限を大きくしすぎるとPDF取得・テキスト抽出に時間がかかり、メモリ使用量も増えるため、営業デモで扱う公開IR資料の範囲に合わせて最小限に設定してください。

...
- 最新財務メトリクス（売上成長率、営業利益率、設備投資額、現預金等の軽量ゲージ付き）（内部値は百万円単位、UI / レポートでは億円・兆円表示）
- 営業優先度スコアと説明、推奨アクション
- 以下の API
  - `GET /api/companies`
...
- 企業詳細 UI
  - 企業プロフィール
  - 最新財務メトリクス（売上成長率、営業利益率、設備投資額、現預金等の軽量ゲージ付き）
  - CRE シグナルカード
  - 根拠テキストと出典参照
...
- 企業別 CRE 営業仮説レポートは、11章構成（エグゼクティブサマリー、優先度判定、スコア内訳、需要兆候、財務所見、経営課題接続、提案テーマ、初回アプローチ、追加ヒアリング、根拠資料、留意事項）に拡張しています。UIでは章番号順に並べ、③スコア内訳と評点理由は表形式で表示します。
